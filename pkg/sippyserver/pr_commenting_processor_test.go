...
func TestMatchPriorRiskAnalysisTest(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"MatchIncompletes": {
			priorRiskAnalysisJSON:    `{"CIJobName":"pull-ci-openshift-origin-master-e2e-aws-ovn-serial","CIJobRunID":1684917111224274944,"Release":"Presubmits","CompareRelease":"4.14","Tests":[],"OverallRisk":{"Level":{"Name":"IncompleteTests","Level":75},"Reasons":["Tests for this run (100) are below the historical average (709):IncompleteTests"]},"OpenBugs":[]}`,
			riskAnalysisJSON:         `{"CIJobName":"pull-ci-openshift-origin-master-e2e-aws-ovn-serial","CIJobRunID":1684985307130236928,"Release":"Presubmits","CompareRelease":"4.14","Tests":[],"OverallRisk":{"Level":{"Name":"IncompleteTests","Level":75},"Reasons":["Tests for this run (57) are below the historical average (709):IncompleteTests"]},"OpenBugs":[]}`,
			expectedRiskLevel:        api.FailureRiskLevelIncompleteTests,
			expectedSummaryTestCount: 0,
		},
		"NoMatchIncompletes": {
			priorRiskAnalysisJSON:    `{"CIJobName":"pull-ci-openshift-origin-master-e2e-aws-ovn-serial","CIJobRunID":1684917111224274944,"Release":"Presubmits","CompareRelease":"4.14","Tests":[],"OverallRisk":{"Level":{"Name":"MissingData","Level":75},"Reasons":["Tests for this run (100) are below the historical average (709):IncompleteTests"]},"OpenBugs":[]}`,
			riskAnalysisJSON:         `{"CIJobName":"pull-ci-openshift-origin-master-e2e-aws-ovn-serial","CIJobRunID":1684985307130236928,"Release":"Presubmits","CompareRelease":"4.14","Tests":[],"OverallRisk":{"Level":{"Name":"IncompleteTests","Level":75},"Reasons":["Tests for this run (57) are below the historical average (709):IncompleteTests"]},"OpenBugs":[]}`,
			expectedRiskLevel:        api.FailureRiskLevelNone,
			expectedSummaryTestCount: 0,
		},
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			var priorRiskAnalysis, riskAnalysis api.CIJobRunRiskAnalysis
			var priorRiskAnalysisPTR *api.CIJobRunRiskAnalysis
...
func TestAnalysisWorker(t *testing.T) {
	...
	prPendingComment := models.PullRequestComment{Org: "openshift", Repo: "origin", PullNumber: 29512, SHA: "8849ed78d4c51e2add729a68a2cbf8551c6d60c9", CIJobRoot: "pr-logs/pull/29512/"} // PR constructed for testing new-test analysis
...
func TestRunCommentAnalysis(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"28075": {
			prCommentProspect: models.PullRequestComment{Org: "openshift", Repo: "origin", PullNumber: 28075, SHA: "79d237196d93eb92ed58c66497d8718259264226", CIJobRoot: "pr-logs/pull/28075/"},
			logLevel:          logrus.DebugLevel,
		},
		"has RA comment already": {
			prCommentProspect: models.PullRequestComment{Org: "openshift", Repo: "origin", PullNumber: 29474, SHA: "58a8615189ebd164a1ce87ffe9b078965a9f4b14", CIJobRoot: "pr-logs/pull/29474/"},
